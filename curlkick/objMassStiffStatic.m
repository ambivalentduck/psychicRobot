...
lArm=L1+L2;
minArm=abs(L1-L2);
xTooLong=max(max(XfromS-lArm,0));
yTooLong=max(max(YfromS-lArm,0));
xTooShort=max(max(minArm-XfromS,0));
yTooShort=max(max(minArm-YfromS,0));
constraints=[xTooLong,xTooShort,yTooLong,yTooShort];
if any(constraints)
    F_cost=inf;
else
    [TBMP,TSP,TNP]=cart2modelStatic(X(fitinds,:),Y(fitinds,:),L1,L2,x0,mass);
    Tnp_sim = TBMP + c*TSP;
...
S_MSE.I_nc      = 4;%no constraints
S_MSE.FVr_ca    = constraints;%no constraint array
