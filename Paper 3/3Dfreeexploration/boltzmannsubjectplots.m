function ofinterest=boltzmannsubjectplots(t,x,v,a,fnumber)
...
% fitmecounts=aPcounts(aPbins<.025);
% fitmebins=aPbins(aPbins<.25);
% xfit=fmincon(@bruteforceexp,1,-1,0)
% 
% fitcounts=1/xfit*exp(-aPbins/xfit);
% fitcounts=fitcounts/sum(fitcounts);
% figure(20)
% bar(aPbins,aPcounts-fitcounts)
...
bar(Pbins,Pcounts)
twosidedexp=1/(2*W)*exp(-abs(Pbins)/W);
twosidedexp=twosidedexp/sum(twosidedexp);
plot(Pbins,twosidedexp,'r','linewidth',2)
...
fitmebins=Usubbins;
xfit=fmincon(@bruteforcegam,[1.5 .2],-eye(2),[-1;-.01])
