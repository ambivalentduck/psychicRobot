...
for k=SUBS
    ...
    for R=1:NR+1
        ...
        for t=1:length(trials)
            ...
            try
                triStruct(t).R2X=sPeak*getR2(trials(t).x(inds,:));
                triStruct(t).R2Y=sPeak*getR2(trials(t).yS(indsy,:));
                triStruct(t).R2YS=sPeak*getR2(trials(t).yS(indsy,:));
                triStruct(t).R2YK=sPeak*getR2(trials(t).yK(indsy,:));
            catch
                thishappened=1
                st1=size(trials(t).x,1);
                triStruct(t).R2X=sPeak*getR2(trials(t).x(inds(1):st1,:));
                st1=size(trials(t).x,1);
                triStruct(t).R2Y=sPeak*getR2(trials(t).yS(indsy(1):st1,:));
                triStruct(t).R2YS=sPeak*getR2(trials(t).yS(indsy(1):st1,:));
                triStruct(t).R2YK=sPeak*getR2(trials(t).yK(indsy(1):st1,:));
            end
        end
        subject(k).R2X=vertcat(triStruct.R2X);
        subject(k).R2YS=vertcat(triStruct.R2YS);
        subject(k).R2YK=vertcat(triStruct.R2YK);

        CBX=1*ones(size(subject(k).R2BXP));
        CBY=2*ones(size(subject(k).R2BXM));
        CX=3*ones(size(subject(k).R2X));
        CYS=4*ones(size(subject(k).R2YS));
        CYK=5*ones(size(subject(k).R2YK));

        Xlist=[CBX;CBY;CX;CYS;CYK];
        Ylist=[subject(k).R2BXP;subject(k).R2BXM;subject(k).R2X;subject(k).R2YS;subject(k).R2YK];
        %Xlist=[CBX;CX;CY];
        %Ylist=[subject(k).R2BX;subject(k).R2X;subject(k).R2Y];
...
%     end
    ...
    errors345(k,:)=halfWidth(stats)';
    errors12(k,:)=halfWidth(stats,.05)';
    errors(k,:)=[errors12(k,1:2) errors345(k,3:5)];
...
%    text(rangemids(k),-.01,num2str(rangemids(k)),'horizontalalignment','center','verticalalignment','top')
    ...
    for S=1:4
        handerrors(k,S)=mean(fullY((fullX==3)&(fullS==S)&~isnan(fullY)));
        intenterrorS(k,S)=mean(fullY((fullX==4)&(fullS==S)&~isnan(fullY)));
        intenterrorK(k,S)=mean(fullY((fullX==5)&(fullS==S)&~isnan(fullY)));
    end
end
% fullC=vertcat(kw.c);
...
fill([rangemids rangemids(end:-1:1)],[mids(:,5)-errors(:,5); mids(end:-1:1,5)+errors(end:-1:1,5)],'w','facecolor',mustard,'edgecolor',mustard)];
for k=1:4
    set(h(k),'EdgeAlpha',ALPHA,'FaceAlpha',ALPHA);
end

for S=1:4
    plot(rangemids,handerrors(:,S),'color',black)
    plot(rangemids,intenterrorS(:,S),'color',red)
    plot(rangemids,intenterrorK(:,S),'color',mustard)
    %text(rangemids(40),intenterrors(40,S),num2str(S))
end
...
ylim([-13 15])
for k=0:100:1000
    text(k,-6.2,num2str(k),'horizontalalignment','center','verticalalignment','top','color',color)
end
plot([0,1000],[0 0],'-','linewidth',2,'color',color)
text(500,-7.4,'Time Post Onset of Disturbing Forces, ms','horizontalalignment','center','verticalalignment','middle','color',color)

Koff=-6;
Kscale=2.5;
for k=1:4
    load(['../Data/Data_pulse/pulse',num2str(k),'W.mat'])
    plot([0 1000],Koff+Kscale*W(end,1)+[0 0],'color',red)
    Wd(Wd<0)=0;
    Wd=[Wd;Wd(end)*ones(100,1)];
    plot(0:5:1000,Koff+Kscale*Wd,'color',mustard)
end
