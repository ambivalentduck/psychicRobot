...
[~,means]=kmeans([clustermeE; clustermeE],3,'emptyaction','singleton','start',[-0.175;-0.026;0.125]);
means=sort(means);
...
for t=1:length(trials)
    [~,starts(t)]=min(abs(trials(t).x(1,1)-means));
    [~,ends(t)]=min(abs(trials(t).x(end,1)-means));
...
for U=1:length(urc)
    ...
    for fk=1:length(f)
        plot(trials(f(fk)).x(:,1),trials(f(fk)).x(:,2)+yoffset*U,'linewidth',.01)
        [~,ind]=findpeaks(abs(trials(f(fk)).f(:,2)),'minpeakheight',5,'npeaks',1);
