function extractDesiredReflexes(name, debug)
...
load(['../Data/',name,'.mat']);
global measuredVals measuredTime x0 fJ getAccel desiredVals desiredTime
...
for k=1:lT
    ...
    for g=1:length(gains)
        ...
        [T,X]=ode45(@armdynamicsInvertedBurdetReflexes,measuredTime,[trials(k).q(inds(1),:)';trials(k).qdot(inds(1),:)']);
        ...
        for kk=1:length(T)
            [dx,torque, reflextorque]=armdynamics_inverted(T(kk),X(kk,:)');
