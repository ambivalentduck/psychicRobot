...
hold on
empiricalSp=sqrt(empiricalE);
[Fe,xe,le,ue]=ecdf(empiricalSp);
h=fill([xe(~isnan(le)); wrev(xe(~isnan(ue)))],[le(~isnan(le)); wrev(ue(~isnan(ue)))],poscolor);
set(h,'edgealpha',0,'facealpha',.5)
plot(xe,Fe,'-','color',poscolor,'linewidth',linewidth)
xlim([-.01 .5])
...
%% Cleanup at the end
annotaxes=axes('position',[0 0 1 1]);
testme=0:.01:1;
%plot(testme,sin(testme),'parent',annotaxes)
set(annotaxes,'visible','off')
