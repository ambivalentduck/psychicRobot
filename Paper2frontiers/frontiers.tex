...
\section{Discussion}
While other visual distortions typically degrade performance and require an adaptation period to overcome, IF immediately enhances performance. The simplest explanation for this is that IF approximates a signal already known to the brain: the path planned for the hand.

This is scientifically relevant as performance enhancement is an active of research. In addition, this finding supports the notion that a particular trajectory is calculated even when explict trajectory constraints are not present. 
