...
The ease with which subjects could make use of their estimated intent provides strong preliminary evidence that a specific desired state trajectory is computed even when moving in a highly variable environment. While the nature of the force disturbance and its high trial-to-trial variability prevent clear characterization of learning, subjects appear to have learned to use their intent within the first exposure. While recent work has identified kinematic constraints irrelevant to the task \citep{mistry2013optimal}, this is the first direct evidence that the entire trajectory is controlled even in the absence of specific instructions or constraints. It remains unclear when the intended trajectory is computed, but variability in the desired trajectory in phases 2 and 4 suggests that the trajectory is planed or at least can be replanned mid-movement. Certainly, it is not finalized before the onset of movement and is not a "strict invariant." Intent determination provides us with a novel and useful window into the nature of motor control.

IF can facilitate human-machine collaboration and artificial performance augmentation by enabling the machine to preserve an operators intent while cancelling unexpected disturbanes from the environment. This would reduce the demands on the human operator increase performance, especially in environments with rapidly changing conditions that can be quickly and accurately measured with electronic instrumentation. This assistance goes beyond environment cancellation by also accounting for any corrections the operator might make based on their expectation of disturbance.

Limitatinos. The stiffness model is shit-tastic. Reaching trajectories, but not tracking, nonlinear, or 3D.
