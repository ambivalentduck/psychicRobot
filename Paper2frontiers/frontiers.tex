...
\title[Running Title]{Reaching is Better When You Get What You Want: Realtime Feedback of Intended Reaching Trajectory Despite an Unstable Forces}
...
Humans often interact with uncertain and complicated environments, such as turbulence, crowds, and traffic. Despite our capacity to learn and adapt, some environments evolve too quickly or with too much uncertainty for meaningful learning. In such an environment, planning a specific route might seem nonsensical. Instead, a continuous cycle of reaction to the environment with no specific intended route might seem more intuitive. We explore the counter-intuitive possibility that a specific route is constructed even when the environment is unstable and unpredictable.

If people try to achieve a goal while minimizing some measure of cost, it is possible to compute a set of rules for reacting to the environment without a specific intended path using optimal feedback control\citep{todorov2002optimal}. This modeling strategy is not necessarily a scientific hypothesis as a measure of cost can always be found \text{a posteriori} that reproduces the behaviors observed and no hypothesized cost function has been claimed as sufficient to describe all reaching. This   but they predict reaction to disturbance. We recently developed an algorithm that could recover the route a person intended to take even if they were forced off of it by environmental disturbances \citep{horowitz2015determining}. Surprisingly, some subjects did not alter their intended route even following exposure to unexpected pulses of force. If humans a plan a route, even if they might change that route in response to disturbance, can they use feedback? Is a route really what they want? Does it take learning?
